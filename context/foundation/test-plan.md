...
| #2 | A Walker (the one who accepted the walk) attempts a transition out of sequence (e.g. start on an already-completed walk, or complete on an accepted walk that was never started) → controller returns a clear HTTP error response; the user is not silently misled | Do not assume "false-return means flash": Phase 1 research found that WalkerWalksController uses a state-scoped find, so the sequential wrong-state attempt raises RecordNotFound → 404, not the else-branch flash. The flash path is race-only. Challenge whether the 404 path is already proven or needs its own same-walker sequential test. | Does any integration test prove the same-walker wrong-state path (e.g. Walker A tries to complete their own accepted walk without starting it first)? Is that covered by Phase 1's IDOR 404 tests or is it a distinct scenario? | Integration test (assert HTTP 404 when legitimate walker attempts an out-of-sequence transition on their own walk) | Writing tests for the else-branch flash path under the assumption it is sequentially reachable — the scoped find prevents it; testing the race path requires concurrency setup |
...
| 2 | State machine feedback | Verify the controller HTTP response for each false-return path in start, complete, and accept | #2 | integration + model | change opened | context/changes/testing-state-machine-feedback/ |
