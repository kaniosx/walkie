...
class OpenRequestsTest < ActionDispatch::IntegrationTest
  setup do
    ...
    @match = open_walk("Rex",  city: "Kraków", postcode: "30-001")
    @other_postcode = open_walk("Pip", city: "Kraków", postcode: "30-999")
    @other_city = open_walk("Fido", city: "Gdańsk", postcode: "30-001")
  end
  ...
  def open_walk(dog_name, city:, postcode:)
    dog = @owner.dogs.create!(name: dog_name, breed: "Labrador")
    dog.walks.create!(owner: @owner, city: city, postcode: postcode)
  end

  test "walker sees only requested walks in their exact city and postcode" do
    accepted = open_walk("Taken", city: "Kraków", postcode: "30-001")
    ...
    assert_not_includes response.body, "Pip",   "different postcode must be hidden"
